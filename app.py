...
if 'np_df' not in st.session_state: st.session_state.np_df = None # State khusus nyimpen data Newspage hasil sedot

# --- 5. CUSTOM CSS (Full Hacker Package) ---
st.markdown("""
    <style>
...
if st.session_state.app_page == "Reconcile":
    ...
    with col1:
        ...
        if st.session_state.np_df is None:
                    ...
                    with st.spinner("Firing up extraction engine..."):
                        ...
                        try:
                            ...
                            with sync_playwright() as p:
                                ...
                                ui_log_np("SERVER", "Awaiting payload generation. Intercept module active (Up to 2 mins)...")
            ...
            st.markdown(make_solid_box("Data Newspage Berhasil Ditarik & Disimpan di Memori!", "#166534", "#4ade80"), unsafe_allow_html=True)
            st.dataframe(st.session_state.np_df.head(3), use_container_width=True) # Kasih ngintip 3 baris
            if st.button("Hapus Data & Ekstrak Ulang", use_container_width=True):
    ...
    if st.session_state.np_df is not None and file2:
        st.divider()
        df1 = st.session_state.np_df # Datanya dapet dari hasil sedotan Playwright
        df2 = load_data(file2)
...
# ─── 7. PAGE: STOCK ADJUSTMENT BOT ───────────────────────────────────────────
    ...
    if run_button:
        ...
        try:
            ...
            st.error("Login Failed: Password salah atau server target sedang tidak merespon (Timeout 30s).")
