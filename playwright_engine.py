...
def run_extract(user_id_np, pass_np, selected_distributor, URL_LOGIN, TIMEOUT_MS, WAREHOUSE, ext_ui_log, alert_callback, supabase, current_user):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.locator("id=pag_FW_SYS_INTF_JOB_btn_Add_Value").wait_for(state="visible", timeout=TIMEOUT_MS)
            ...
            page.locator("id=pag_FW_DisclaimerMessage_btn_okay_Value").wait_for(state="attached", timeout=TIMEOUT_MS)
            ext_ui_log("SYS", "Bypassing disclaimer prompt...")
            page.locator("id=pag_FW_DisclaimerMessage_btn_okay_Value").click(force=True)
            page.locator("id=pag_FW_SYS_INTF_JOB_DTL_PopupNew_INTF_ID_SelectButton").wait_for(state="attached", timeout=TIMEOUT_MS)
            ...
            page.locator("id=pop_Dynamic_gft_List_2_FilterField_Value").wait_for(state="visible", timeout=TIMEOUT_MS)
            ...
            page.get_by_text("E_20150315090000028", exact=True).wait_for(state="visible", timeout=TIMEOUT_MS)
            ...
            page.locator("id=pag_FW_SYS_INTF_JOB_DTL_PopupNew_FILE_TYPE_Value").wait_for(state="attached", timeout=TIMEOUT_MS)
            ...
            page.locator("id=pag_FW_SYS_INTF_JOB_RootNew_btn_Save_Value").wait_for(state="attached", timeout=TIMEOUT_MS)
...
def run_execution(df_view, bot_user, bot_pass, selected_distributor, URL_LOGIN, TIMEOUT_MS, WAREHOUSE, REASON_CODE, TABLE_UPDATE_INTERVAL, ui_log, alert_callback, table_placeholder, log_label_placeholder, supabase):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.locator("id=pag_InventoryRoot_tab_Main_itm_StkAdj").wait_for(state="attached", timeout=TIMEOUT_MS)
